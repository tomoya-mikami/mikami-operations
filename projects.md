...
| P-011 | カーパーツ管理ツール | 運用開始前確認 / 価格監視MVP・注文コスト突合MVP・作業マニュアル作成済み | 外注リサーチャーのGoogle Sheets取り込み、禁止/重複チェック、SKU採番、Sellsta CSV、給与計算、モノタロウ価格変動・取扱終了検知、eBay注文データ、部品商直送請求明細、送料差額、関税請求書、Chatwork品切れ/値上げ情報、初心者向け作業標準化を進める | `carparts-manager-import-summary.md`, `carparts-price-monitor-roadmap.md`, `carparts-cost-control-roadmap.md`, `carparts-price-monitor-mvp-implementation.md`, `carparts-research-operator-manual.md`, `carparts-research-improvement-proposal.md`, `gas/carparts_price_monitor_mvp.gs`, `gas/carparts_cost_control_mvp.gs`, `tool-inventory.md` | ログイン済みGoogle Sheets環境またはAlfで、既存入力用シートを壊さずGASを別ファイル追加し、価格監視・コスト監視の初期セットアップ、10件/50件/100件テストを行う。eBay注文、部品商請求明細、関税請求書が `注文コスト突合` で注文ID/SKUごとにつながり、送料差額、関税差額、Chatwork貼付情報が `出品反映候補` に集約されるか確認し、パートさん用マニュアルへ展開する。Sellsta CSV実出品テスト、未確認の外注担当者シート、FastAPI起動、担当者DB、GCP認証情報、Sheets権限、CORS/認証設定もAlfで確認する |
...
| P-013 | 外注管理・報酬計算 | 整理中 | 外注スタッフとのやり取り、作業管理、報酬計算、管理シート、ツール化を整理する | `outsourcing-ops-chat-prompt.md`, `outsourcing-management-improvement-plan.md` | 現行表の構造を参考に、スタッフ/作業単価/作業実績/月次報酬/支払チェック/エラー検出の分離案を検討する |

## 現在の最優先

1. T-017: Shipping Policy v9 / Rate Tableの未確認項目を、xlsx本体またはeBay画面で確認する。
2. T-023/T-026: カーパーツ管理ツールの運用開始前チェックと、モノタロウ価格監視MVPを進める。価格監視は5,000件を標準14日で1周し、設定タブで周期変更できるようにする。
