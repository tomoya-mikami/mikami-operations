...
| P-011 | カーパーツ管理ツール | 運用開始前確認 / 価格監視MVP・作業マニュアル作成済み | 外注リサーチャーのGoogle Sheets取り込み、禁止/重複チェック、SKU採番、Sellsta CSV、給与計算、モノタロウ価格変動・取扱終了検知、初心者向け作業標準化を進める | `carparts-manager-import-summary.md`, `carparts-price-monitor-roadmap.md`, `carparts-price-monitor-mvp-implementation.md`, `carparts-research-operator-manual.md`, `carparts-research-improvement-proposal.md`, `gas/carparts_price_monitor_mvp.gs`, `tool-inventory.md` | ログイン済みGoogle Sheets環境またはAlfで、既存入力用シートを壊さずGASを別ファイル追加し、初期セットアップ、監視マスター作成、10件/50件/100件テストを行う。あわせて作業マニュアルをパートさん用に展開し、Sellsta CSV実出品テスト、未確認の外注担当者シート、FastAPI起動、担当者DB、GCP認証情報、Sheets権限、CORS/認証設定をAlfで確認する |
