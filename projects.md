...
| P-011 | カーパーツ管理ツール | 運用開始前確認 / 価格監視MVP・送料関税Chatwork反映MVP・作業マニュアル作成済み | 外注リサーチャーのGoogle Sheets取り込み、禁止/重複チェック、SKU採番、Sellsta CSV、給与計算、モノタロウ価格変動・取扱終了検知、送料差額、関税差額、Chatwork品切れ/値上げ情報、初心者向け作業標準化を進める | `carparts-manager-import-summary.md`, `carparts-price-monitor-roadmap.md`, `carparts-cost-control-roadmap.md`, `carparts-price-monitor-mvp-implementation.md`, `carparts-research-operator-manual.md`, `carparts-research-improvement-proposal.md`, `gas/carparts_price_monitor_mvp.gs`, `gas/carparts_cost_control_mvp.gs`, `tool-inventory.md` | ログイン済みGoogle Sheets環境またはAlfで、既存入力用シートを壊さずGASを別ファイル追加し、価格監視・コスト監視の初期セットアップ、10件/50件/100件テストを行う。送料差額、関税差額、Chatwork貼付情報が `出品反映候補` に集約されるか確認し、パートさん用マニュアルへ展開する。Sellsta CSV実出品テスト、未確認の外注担当者シート、FastAPI起動、担当者DB、GCP認証情報、Sheets権限、CORS/認証設定もAlfで確認する |
