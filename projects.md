...
| P-011 | カーパーツ管理ツール | 運用開始前確認 / 価格監視ロードマップ作成済み | 外注リサーチャーのGoogle Sheets取り込み、禁止/重複チェック、SKU採番、Sellsta CSV、給与計算、モノタロウ価格変動・取扱終了検知を自動化する | `carparts-manager-import-summary.md`, `carparts-price-monitor-roadmap.md`, `tool-inventory.md` | 既存入力用シートを壊さず、設定/価格監視マスター/価格履歴/販売価格見直し候補の別タブ方式でMVPを作る。あわせてSellsta CSV実出品テスト、未確認の外注担当者シート、FastAPI起動、担当者DB、GCP認証情報、Sheets権限、CORS/認証設定をAlfで確認する |
