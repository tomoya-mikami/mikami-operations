...
## 2026-05-26 Alf追記: 許可後の実行結果

三神さんから、T-020のマスク済み個別移動予定リスト作成と、T-024の別シート/別タブ試作について許可あり。

完了:

- T-020: `ebay-phase1-needs-review-masked-move-list.md` を作成。50件を `NR-001` から `NR-050` に番号化し、推定月、分類、移動先候補、扱い、確度を整理。実移動は未実施。
- T-024: `spreadsheets/management-defense-dashboard-v1-prototype.xlsx` を作成。経営サマリー、月次利益、データ取込状況、支出サブスク、商品カテゴリ別、要確認、月次チェック、設定を含む。
- T-024: `spreadsheets/management-defense-dashboard-v1-preview.png` を作成。
...
- T-024: Google Sheets化する前に、実列名を確認する。
- T-020: マスク済みリストを三神さん/Fayが確認し、実移動するか、Claudeに整理を渡すかを決める。
...
| T-028 | カーパーツ送料差額・関税・Chatwork情報反映MVP | Fay司令塔 / Alf実装候補 | MVPコード作成済み / 本番追加待ち | 推定送料と実送料の差、部品商直送請求明細、関税請求書、eBay注文データ、Chatworkコミュニティ内の品切れ/値上げ/廃番情報を `出品反映候補` へ集約し、赤字出品を防ぐ | `carparts-cost-control-roadmap.md`, `gas/carparts_cost_control_mvp.gs`, `visuals/carparts-cost-control-loop.svg`, `alf-carparts-cost-control-mvp-request.md`, `carparts-research-operator-manual.md`, `carparts-research-improvement-proposal.md` | ログイン済みGoogle Sheets環境で、既存入力用シートを壊さずGASを別ファイル追加する。`eBay注文取込`、`部品商請求明細`、`送料差額チェック`、`関税チェック`、`Chatwork情報取込` を各10件でテストし、`注文コスト突合` と `出品反映候補` に候補が追加されるか確認する。Chatwork APIトークンやSecrets値は保存せず、初期MVPは手動貼付解析で進める。価格改定・出品停止・eBay/Sellsta直接反映は候補表示までで自動実行しない | 進行中 |
