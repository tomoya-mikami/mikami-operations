...
| T-028 | カーパーツ送料差額・関税・Chatwork情報反映MVP | Fay司令塔 / Alf実装候補 | MVPコード作成済み / 本番追加待ち | 推定送料と実送料の差、請求関税の妥当性、Chatworkコミュニティ内の品切れ/値上げ/廃番情報を `出品反映候補` へ集約し、赤字出品を防ぐ | `carparts-cost-control-roadmap.md`, `gas/carparts_cost_control_mvp.gs`, `visuals/carparts-cost-control-loop.svg`, `alf-carparts-cost-control-mvp-request.md`, `carparts-research-operator-manual.md`, `carparts-research-improvement-proposal.md` | ログイン済みGoogle Sheets環境で、既存入力用シートを壊さずGASを別ファイル追加する。`送料差額チェック`、`関税チェック`、`Chatwork情報取込` を各10件でテストし、`出品反映候補` に候補が追加されるか確認する。Chatwork APIトークンやSecrets値は保存せず、初期MVPは手動貼付解析で進める。価格改定・出品停止・eBay/Sellsta直接反映は候補表示までで自動実行しない | 進行中 |
