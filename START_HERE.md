...
- 引き継ぎファイルの作成・更新
- 台帳、ログ、プロジェクトメモの更新
- 新規ファイル、設計書、ドラフトコード、テストコード、作業用チェックリストの作成
- 既存データを壊さない読み取り確認
- 既存列や既存データを上書きしない新規シート/新規タブ追加
- 候補リスト、見直し候補、アラート一覧、CSV案の作成
- Gitの `status` 確認
- 変更内容のcommit
- GitHubへのpush
- Alfへ渡す依頼文の作成
...
- APIキー、認証JSON、Secrets、トークン、サービスアカウント情報などの確認・保存・表示
- Google Drive内の削除・移動
- 価格改定、出品停止、既存出品更新など、外部サービスや本番業務データへ直接反映する操作

迷った場合:

- 元データを壊さない別ファイル/別タブ/候補リストとして作れるなら、確認せず進める。
- 削除、不可逆、セキュリティ、個人情報、認証情報、本番データ上書きに当たる場合だけ止めて確認する。
